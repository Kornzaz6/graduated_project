<template>
  <div class="min-h-screen p-8 bg-gray-100">
    <div class="max-w-6xl mx-auto">
      <h1 class="mb-8 text-2xl font-bold">รายการบิลรายเดือน</h1>

      <!-- Loading -->
      <div v-if="loading" class="py-20 text-center text-gray-500">กำลังโหลดข้อมูล...</div>
        ...
        >
          <div class="flex items-center justify-between mb-4">
            <div>
              ...
              <div class="text-sm text-gray-500">ครบกำหนด {{ formatDate(payment.dueDate) }}</div>
            </div>

            <div class="text-right">
              <div class="text-xl font-bold">{{ payment.amount.toLocaleString() }} ฿</div>

              <span class="status-badge" :class="statusColor(payment.status)">
          ...
          <div class="flex items-center gap-4">
            ...
            <!-- Upload -->
            <label v-if="canUpload(payment.status)" class="cursor-pointer secondary-btn">
              อัปโหลดสลิป
              <input type="file" class="hidden" @change="handleFileUpload($event, payment.id)" />
            </label>
...
const fetchPayments = async () => {
  try {
    ...
    if (!contractId && currentUser?.id) {
      const res = await api.get(`/rental/member/${currentUser.id}`, { headers: getAuthHeaders() })

      const requests = res.data

      const activeRequest = requests.find((r: any) => r.leaseContract?.status === 'ACTIVE')
    ...
    const res = await api.get(`/payments/contract/${contractId}`, { headers: getAuthHeaders() })
...
const sortedPayments = computed(() => {
  return [...payments.value].sort(
    (a, b) => new Date(a.billingMonth).getTime() - new Date(b.billingMonth).getTime()
  )
})
...
const openQR = async (payment: any) => {
  try {
    ...
    const res = await api.get(`/payments/contract/${payment.contractId}/qr`, {
...
const handleFileUpload = async (event: any, paymentId: number) => {
  ...
  try {
    ...
    await api.post(`/payments/${paymentId}/upload-slip`, formData, {
      headers: {
...
const canUpload = (status: string) => status === 'PENDING' || status === 'REJECTED'

const canGenerateQR = (status: string) => status === 'PENDING' || status === 'REJECTED'
