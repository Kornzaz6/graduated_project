  ...
  <div v-if="dormitory" class="min-h-screen bg-gray-100">
    ...
    <div class="max-w-6xl px-6 mx-auto -mt-10">
      <div class="overflow-hidden bg-white shadow-xl rounded-2xl">
        <div v-if="dormitory.images && dormitory.images.length" class="relative h-[420px]">
          <img :src="dormitory.images[currentImage].imageUrl" class="object-cover w-full h-full" />
    ...
    <div class="max-w-6xl px-6 py-10 mx-auto">
        ...
        <div v-if="dormitory.rooms.length" class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          ...
          >
            <div class="flex justify-between mb-3">
              <h3 class="font-semibold">ห้อง {{ room.roomNumber }}</h3>
      ...
      <!-- REVIEW FORM -->
      <section v-if="isMember && hasRented" class="mb-12">
        <div class="p-6 bg-white shadow rounded-2xl">
          <h3 class="mb-4 text-lg font-semibold">เขียนรีวิว</h3>

          <!-- stars -->
          <div class="flex mb-4 text-2xl text-yellow-400">
            <span v-for="n in 5" :key="n" class="cursor-pointer" @click="rating = n">
              {{ n <= rating ? '★' : '☆' }}
            </span>
          </div>
          ...
          <button
            @click="submitReview"
            class="px-6 py-2 mt-4 text-white rounded-xl"
            :class="hasRented ? 'bg-blue-600 hover:bg-blue-700' : 'bg-gray-400 cursor-not-allowed'"
            :disabled="!hasRented"
          >
            ส่งรีวิว
          </button>
      ...
      <!-- REVIEW LIST -->
      <section>
        <h2 class="mb-6 text-2xl font-bold">รีวิวจากผู้พัก</h2>
        ...
        <div v-else class="p-10 text-center text-gray-500 bg-white rounded-xl">ยังไม่มีรีวิว</div>
      </section>

      <!-- NOT ALLOWED REVIEW -->
      <div
        v-if="isMember && !hasRented"
        class="p-6 mb-12 text-center text-gray-500 bg-white shadow rounded-2xl"
      >
        เฉพาะผู้ที่เคยเช่าห้องพักเท่านั้นจึงจะสามารถรีวิวได้
      </div>
    </div>
  </div>
</template>
...
const checkRentalHistory = async () => {
  ...
  try {
    const { data } = await api.get(`/rental/member/${currentUser.value.id}`)

    const rentals = Array.isArray(data) ? data : []

    hasRented.value = rentals.some((r: any) => r.room?.dormitoryId === dormitory.value.id)
  } catch (error) {
...
const averageRating = computed(() => {
  ...
  const total = dormitory.value.reviews.reduce((sum: number, r: any) => sum + r.rating, 0)

  return total / dormitory.value.reviews.length
})

const fetchDormitory = async () => {
  const { data } = await api.get(`/dormitories/${route.params.id}`)

  dormitory.value = data

  await checkRentalHistory()
}
...
const submitReview = async () => {
  ...
  try {
    ...
    alert(err.response?.data?.message || 'ไม่สามารถรีวิวได้')
  }
}
...
const nextImage = () => {
  currentImage.value = (currentImage.value + 1) % dormitory.value.images.length
}

const prevImage = () => {
  currentImage.value =
    (currentImage.value - 1 + dormitory.value.images.length) % dormitory.value.images.length
}
