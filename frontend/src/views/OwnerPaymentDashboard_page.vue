<template>
  <div class="min-h-screen p-8 bg-gray-100">
    <div class="p-8 mx-auto bg-white shadow-xl max-w-7xl rounded-2xl">
      <h1 class="mb-8 text-2xl font-bold">Payment Management</h1>
      ...
      <div class="p-6 mb-10 bg-gray-50 rounded-xl">
        ...
        <div class="grid grid-cols-4 gap-4">
          ...
          <input v-model="newPayment.amount" type="number" placeholder="Amount" class="input" />

          <input v-model="newPayment.dueDate" type="date" class="input" />
      ...
      <div v-if="loading" class="py-10 text-center text-gray-500">Loading payments...</div>
  ...
  <span v-else class="text-gray-400">-</span>
          ...
          <tr v-if="payments.length === 0">
            <td colspan="6" class="py-10 text-center text-gray-400">No payments yet</td>
          </tr>
        </tbody>
      </table>
    </div>
...
const fetchContracts = async () => {
  ...
  try {
    const res = await api.get(`/owners/${user.id}/contracts`, { headers: getAuthHeaders() })
...
const createPayment = async () => {
  if (!selectedContractId.value || !newPayment.value.amount || !newPayment.value.dueDate) {
