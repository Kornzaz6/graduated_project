  ...
  <div class="min-h-screen bg-gray-50">
    ...
    <header class="sticky top-0 z-[2000] border-b bg-white/80 backdrop-blur-md">
      <div class="flex items-center justify-between px-6 py-4 mx-auto max-w-7xl">
        <!-- Logo -->
        <div class="text-2xl font-bold tracking-tight cursor-pointer text-rose-500" @click="goHome">
          DormitoryHub
        </div>
          ...
          <div class="relative menu-wrapper">
            ...
            <transition name="fade">
              ...
              >
                ...
                <!-- ADMIN -->
                <router-link v-if="isAdmin" :to="{ name: 'AdminDashboard' }" class="dropdown-item">
                  Admin Dashboard
                </router-link>

                <router-link v-if="isAdmin" :to="{ name: 'ManageUsers' }" class="dropdown-item">
                  Manage Users
                </router-link>
                ...
                <router-link v-if="isAdmin" :to="{ name: 'ManageOwners' }" class="dropdown-item">
                  Manage Owners
                </router-link>

                <router-link v-if="isAdmin" :to="{ name: 'DormApproval' }" class="dropdown-item">
                  Dormitory Approval
                </router-link>

                <!-- OWNER -->
                <router-link v-if="isOwner" :to="{ name: 'OwnerDashboard' }" class="dropdown-item">
                  Owner Dashboard
                </router-link>

                <router-link v-if="isOwner" :to="{ name: 'OwnerPayments' }" class="dropdown-item">
                  Payment Dashboard
                </router-link>

                <!-- MEMBER -->
                <router-link v-if="isMember" :to="{ name: 'MemberHome' }" class="dropdown-item">
                  Home
                </router-link>
                ...
                <router-link v-if="isMember" :to="{ name: 'MemberPayments' }" class="dropdown-item">
                  View All Payments
                </router-link>

                <router-link v-if="isMember" :to="{ name: 'applyowner' }" class="dropdown-item">
                  Apply to be Owner
                </router-link>
                ...
                <router-link v-if="isMember" :to="{ name: 'MemberTickets' }" class="dropdown-item">
                  Contact Owner
                </router-link>
...
onMounted(async () => {
  ...
  if (isMember.value && currentUser.value?.id) {
    try {
      ...
      const approvedList = list.filter((r: any) => r.status === 'APPROVED' && !r.leaseContract)
